import { normalizeUri, isResolvableUri } from '../src/index'
...
describe('normalizeUri', () => {
  ...
  describe('redundant path segments', () => {
  ...
  describe('embedded gateway URLs', () => {
    it('extracts IPFS CID from embedded gateway in ipfs:// URI', () => {
      expect(normalizeUri('ipfs://https://gateway.pinata.cloud/ipfs/QmXxx')).toBe('ipfs://QmXxx')
    })

    it('extracts IPFS CID with path from embedded gateway', () => {
      expect(normalizeUri('ipfs://https://ipfs.io/ipfs/QmXxx/metadata.json')).toBe('ipfs://QmXxx/metadata.json')
    })

    it('extracts IPNS name from embedded gateway in ipns:// URI', () => {
      expect(normalizeUri('ipns://https://ipfs.io/ipns/example.eth')).toBe('ipns://example.eth')
    })

    it('handles http embedded gateway', () => {
      expect(normalizeUri('ipfs://http://localhost:8080/ipfs/QmXxx')).toBe('ipfs://QmXxx')
    })

    it('falls back to redundant-path fix when embedded URL has no gateway path', () => {
      expect(normalizeUri('ipfs://ipfs/QmXxx')).toBe('ipfs://QmXxx')
    })
  })
